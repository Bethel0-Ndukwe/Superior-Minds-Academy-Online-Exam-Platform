// =============================================
// COMPLETE JAVASCRIPT CODE - ALL FIXES INCLUDED
// =============================================
...
// =============================================
// 3. GOOGLE SHEETS AUTO-SAVE FUNCTION (FIXED)
// =============================================
...
async function autoSaveToGoogleSheets(result) {
    ...
    try {
        ...
        // IMPORTANT: Use mode: 'no-cors' for Google Apps Script
        const response = await fetch(webAppUrl, {
            method: 'POST',
            mode: 'no-cors', // This fixes "Failed to fetch" error
            headers: {
...
// =============================================
// 4. UPDATED SUBMIT EXAM FUNCTION
// =============================================
...
async function submitExam() {
    ...
    // Display results (SHOW GRADE NOT PERCENTAGE TO STUDENTS)
    displayResults(score);
    examState.isExamActive = false;
}

// =============================================
// 5. UPDATED DISPLAY RESULTS FUNCTION
// =============================================
...
function displayResults(score) {
    ...
    // For STUDENTS: Show grade and score, NOT percentage
...
// =============================================
// 6. UPDATED UNANSWERED QUESTIONS HANDLING
// =============================================
...
// =============================================
// 8. FIXED RESULTS DISPLAY IN ADMIN PANEL
// =============================================
...
function loadAdminResults() {
    ...
    setTimeout(() => {
        ...
        // REMOVE DUPLICATES - FIX FOR DISPLAYING TWICE
...
async function loadQuestions(classId, subject) {
    try {
        ...
        // Use loaded questions if available
        if (appData.questions.length === 0) {
            console.log('No questions loaded, trying to sync');
...
async function startExam(classId, subject) {
    ...
    try {
        ...
        examSubject.textContent = `${subject} - ${classId.replace('-', ' ')}`;
...
function updateStudentDashboard() {
    ...
    classCard.innerHTML = `
        ...
        <p>Welcome, Student</p>
        <p>ID: ${examState.studentInfo.admissionNumber.substring(0, 3)}***</p>
...
async function validateStudent(admissionNumber, fullName, className) {
    try {
        // If no students data loaded, allow any (demo mode)
        if (appData.students.length === 0) {
            console.log('No students loaded, using demo mode');
            return {
                valid: true,
                student: {
                    AdmissionNumber: admissionNumber,
...
console.log('Superior Mind Academy Exam Portal loaded with all fixes');
